...
void ice_trace9(const struct trace_entry * __entry, int __a, 
				...
				int __f, int __g, int __h, int __i)
{
	...
	while ((16 - ((head - ice_comm_blk.tx_tail) & 0xffff)) < 10);
...
void dcclog9(const struct trace_entry *, int, 
			 int,  int, int, int,
			 int,  int, int, int) 
	__attribute__ ((weak, alias ("ice_trace9")));
