...
void ice_trace4(const struct dcc_trace_entry * __entry, int __a, 
				int __b, int __c, int __d)
{
	...
	head = comm->tx_head;
	while ((16 - ((head - comm->tx_tail) & 0xffff)) < 5);
...
void dcclog4(const struct dcc_trace_entry *, int, 
			 int, int, int) 
	__attribute__ ((weak, alias ("ice_trace4")));
