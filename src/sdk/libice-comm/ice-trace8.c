...
void ice_trace8(const struct dcc_trace_entry * __entry, int __a, 
			 ...
			 int __f, int __g, int __h)
{
	...
	head = comm->tx_head;
	while ((16 - ((head - comm->tx_tail) & 0xffff)) < 9);
...
void dcclog8(const struct dcc_trace_entry *, int, 
			 int,  int, int, int,
			 int, int, int) 
	__attribute__ ((weak, alias ("ice_trace8")));
