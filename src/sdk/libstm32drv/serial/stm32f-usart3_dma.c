...
struct stm32f_serial_dma_drv usart3_serial_dma_drv = {
	.uart = STM32_USART3
};

void stm32f_usart3_isr(void)
{
	stm32f_serial_dma_isr(&usart3_serial_dma_drv);
}

void stm32f_dma1_stream1_isr(void)
{
	stm32f_serial_dma_rx_isr(&usart3_serial_dma_drv);
}

const struct serial_dev usart3_serial_dma_dev = {
	.drv = &usart3_serial_dma_drv,
...
struct serial_dev * stm32f_uart3_serial_dma_init(unsigned int baudrate, 
												 unsigned int flags)
{
	struct stm32f_serial_dma_drv * drv = &usart3_serial_dma_drv;
...
#ifdef THINKAPP
	thinkos_irq_register(STM32_IRQ_USART3, SERIAL_IRQ_PRIORITY, 
						 stm32f_usart3_isr);

	thinkos_irq_register(STM32F_IRQ_DMA1_STREAM1, SERIAL_IRQ_PRIORITY, 
						 stm32f_dma1_stream1_isr);
#else
	cm3_irq_pri_set(STM32_IRQ_USART3, SERIAL_IRQ_PRIORITY);
	...
	return (struct serial_dev *)&usart3_serial_dma_dev;
}
