...
struct stm32f_serial_dma_drv uart4_serial_dma_drv = {
	.uart = STM32_UART4
};

void stm32f_uart4_isr(void)
{
	stm32f_serial_dma_isr(&uart4_serial_dma_drv);
}

void stm32f_dma1_stream2_isr(void)
{
	stm32f_serial_dma_rx_isr(&uart4_serial_dma_drv);
}

const struct serial_dev uart4_serial_dma_dev = {
	.drv = &uart4_serial_dma_drv,
