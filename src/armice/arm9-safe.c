...
int arm9_wr8(jtag_tap_t * tap, uint32_t addr, uint8_t data)
{
	...
	DCC_LOG2(LOG_INFO, "[%08x] <- %02x", addr, data);

	jtag_arm9_step(tap, 1, ARM_LDM(ARM_R1_R2, ARM_PC));
//	jtag_arm9_step(tap, 2, ARM_NOP);
	jtag_arm9_step_wr(tap, 3, data);
	jtag_arm9_step_wr(tap, 1, addr);

	/* run at system speed  */
	return jtag_arm9_step_sys(tap, ARM_STRB(ARM_R1, ARM_R2, 0));
}
...
int arm9_rd8(jtag_tap_t * tap, uint32_t addr, uint8_t * data)
{
	...
	jtag_arm9_step(tap, 1, ARM_LDR(ARM_R2, ARM_PC, 0));
//	jtag_arm9_step(tap, 2, ARM_NOP);
	jtag_arm9_step_wr(tap, 3, addr);

	/* run at system speed and */
	ret = jtag_arm9_step_sys(tap, ARM_LDRB(ARM_R1, ARM_R2, 0));

	jtag_arm9_step(tap, 1, ARM_STR(ARM_R1, ARM_PC, 0));
	jtag_arm9_step(tap, 2, ARM_NOP);
...
{
	DCC_LOG2(LOG_INFO, "[%08x] <- %04x", addr, data);
	...
	/* run at system speed  */
	return jtag_arm9_step_sys(tap, ARM_STRH(ARM_R1, ARM_R2, 0));
}
...
int arm9_rd16(jtag_tap_t * tap, uint32_t addr, uint16_t * data)
{
	...
	jtag_arm9_step(tap, 1, ARM_LDR(ARM_R1_R2, ARM_PC, 0));
							...
							struct armice_context * ct)
{
	...
	jtag_arm9_step(tap, 1, ARM_STR(ARM_R0, ARM_PC, 0));
	jtag_arm9_step(tap, 2, ARM_NOP);
	jtag_arm9_step_rd(tap, 1, &ct->cpsr);
	jtag_arm9_step(tap, 2, ARM_NOP);
...
void arm9_context_restore(jtag_tap_t * tap, armice_context_t * ct)
{
	...
	DCC_LOG1(LOG_TRACE, "restoring cpsr=0x%08x", ct->cpsr);
	jtag_arm9_step(tap, 1, ARM_MSR_CPSR_IM(ct->cpsr & 0xff, 0, 1));
	jtag_arm9_step(tap, 4, ARM_NOP);
	jtag_arm9_step(tap, 1, ARM_MSR_CPSR_IM((ct->cpsr >> 8) & 0xff, 12, 2));
	jtag_arm9_step(tap, 4, ARM_NOP);
	jtag_arm9_step(tap, 1, ARM_MSR_CPSR_IM((ct->cpsr >> 16) & 0xff, 8, 4));
	jtag_arm9_step(tap, 4, ARM_NOP);
	jtag_arm9_step(tap, 1, ARM_MSR_CPSR_IM((ct->cpsr >> 24) & 0xff, 4, 8));
	jtag_arm9_step(tap, 2, ARM_NOP);
	...
	jtag_arm9_step(tap, 2, ARM_NOP);
	jtag_arm9_step_wr(tap, 1, ct->pc - (1 * 4));
	jtag_arm9_step(tap, 3, ARM_NOP);
}
...
int arm9_insn_step(jtag_tap_t * tap, struct armice_context * ct)
{
	...
	ret = jtag_arm9_step_sys(tap, ARM_B(-(6 * 4)));
					...
					uint32_t * regv, int regc)
{
	...
	jtag_arm9_step(tap, 1, ARM_LDM(regs | (1 << ARM_PC), ARM_PC));
	jtag_arm9_step(tap, 2, ARM_NOP);
	for (i = 0; i < regc; i++) {
	...
	/* resume */
	jtag_arm9_step(tap, 1, ARM_B(-(4 * 4)));
							 ...
							 armice_context_t * ct)
{
	...
	jtag_arm9_step_thumb_rd(tap, 1, &ct->r[0]);

	jtag_arm9_step(tap, 1, THUMB_MOV(ARM_R0, ARM_PC));
	jtag_arm9_step(tap, 1, THUMB_STR(ARM_R0, ARM_R0));
	jtag_arm9_step(tap, 2, THUMB_NOP);
	jtag_arm9_step_thumb_rd(tap, 1, &ct->pc);
	...
	/* store R0 (CPSR) */
	jtag_arm9_step(tap, 1, ARM_STR(ARM_R0, ARM_PC, 0));
	jtag_arm9_step(tap, 1, ARM_EOR(ARM_R0, ARM_R0, ARM_R0));
	jtag_arm9_step(tap, 1, ARM_NOP);
	jtag_arm9_step_rd(tap, 1, &ct->cpsr);
...
void arm9_thumb_context_restore(jtag_tap_t * tap, armice_context_t * ct)
{
	...
	jtag_arm9_step(tap, 1, ARM_LDM(ARM_R0_LR, ARM_SP));
	jtag_arm9_step(tap, 2, ARM_NOP);
	/* R0 = PC */
	...
	jtag_arm9_step(tap, 1, THUMB_NOP);
}
			   ...
			   uint32_t offs, void * p, int len)
{
	...
	/* uses R14 as index */
	jtag_arm9_step(tap, 1, ARM_LDR(ARM_R14, ARM_PC, 0));
//	jtag_arm9_step(tap, 2, ARM_NOP);
	jtag_arm9_step_wr(tap, 3, addr);
	...
	if(addr & 0x01) {
		...
		jtag_arm9_step_sys(tap, ARM_LDRB(ARM_R0, ARM_R14, 1));
		jtag_arm9_step(tap, 1, ARM_STR(ARM_R0, ARM_PC, 0));
		jtag_arm9_step(tap, 2, ARM_NOP);
		jtag_arm9_step_rd(tap, 1, &data);
		*cp++ = data;
	...
	if ((len >= 2) && (addr & 0x02)) {
		...
		jtag_arm9_step_sys(tap, ARM_LDRH(ARM_R0, ARM_R14, 2));
		jtag_arm9_step(tap, 1, ARM_STR(ARM_R0, ARM_PC, 0));
		jtag_arm9_step(tap, 2, ARM_NOP);
		jtag_arm9_step_rd(tap, 1, &data);
		*cp++ = data;
	...
	while (len >= 4) {
		...
		if (n == 1) {
			...
			jtag_arm9_step_sys(tap, ARM_LDR(ARM_R0, ARM_R14, 4));
			jtag_arm9_step(tap, 1, ARM_STR(ARM_R0, ARM_PC, 0));
			jtag_arm9_step(tap, 2, ARM_NOP);
			jtag_arm9_step_rd(tap, 1, &data);
			*cp++ = data;
			...
			*cp++ = data >> 24;
			...
			jtag_arm9_step_sys(tap, ARM_LDMIA((1 << n) - 1, ARM_R14));
			jtag_arm9_step(tap, 1, ARM_STM((1 << n) - 1, ARM_PC));
			jtag_arm9_step(tap, 2, ARM_NOP);
			for (j = 0; j < n; j++) {
				jtag_arm9_step_rd(tap, 1, &data);
				*cp++ = data;
	...
	if(len & 0x01) {
		/* run at system speed */
		jtag_arm9_step_sys(tap, ARM_LDRB(ARM_R0, ARM_R14, 0));
		jtag_arm9_step(tap, 1, ARM_STR(ARM_R0, ARM_PC, 0));
		jtag_arm9_step(tap, 2, ARM_NOP);
		jtag_arm9_step_rd(tap, 1, &data);
		*cp = data;
...
//	jtag_arm9_step(tap, 2, ARM_NOP);
	...
	if (addr & 0x01) {
		data = *cp++;
		jtag_arm9_step(tap, 1, ARM_LDR(ARM_R0, ARM_PC, 0));
		jtag_arm9_step(tap, 2, ARM_NOP);
		jtag_arm9_step_wr(tap, 1, data);
		/* run at system speed and increment the base register by one */
		jtag_arm9_step_sys(tap, ARM_STRB(ARM_R0, ARM_R14, 1));
		len--;
		addr++;
		cnt++;
	}
	...
	if ((len >= 2) && (addr & 0x02)) {
		...
		jtag_arm9_step_wr(tap, 1, data);
		/* run at system speed and increment the base register by two */
		jtag_arm9_step_sys(tap, ARM_STRH(ARM_R0, ARM_R14, 2));
		len -= 2;
		cnt += 2;
	}
...
//			jtag_arm9_step(tap, 2, ARM_NOP);
			jtag_arm9_step_wr(tap, 3, data);
			/* run at system speed and increment the base register by 4 */
			jtag_arm9_step_sys(tap, ARM_STR(ARM_R0, ARM_R14, 4));
		} else {
			jtag_arm9_step(tap, 1, ARM_LDM((1 << n) - 1, ARM_PC));
//			jtag_arm9_step(tap, 2, ARM_NOP);
			data = *cp++;
			data += *cp++ << 8;
			data += *cp++ << 16;
			...
			/* run at system speed and increment the base register by (4 * n) */
			jtag_arm9_step_sys(tap, ARM_STMIA((1 << n) - 1, ARM_R14));
		}
		len -= 4 * n;
		cnt += 4 * n; 
	}
	...
	if (len) {
		...
		/* run at system speed */
		jtag_arm9_step_sys(tap, ARM_STRB(ARM_R0, ARM_R14, 0));
		cnt++;
	}
