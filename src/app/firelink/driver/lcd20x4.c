...
static void lcd_out(unsigned int data) 
{
	...
	/* EN pulse */
	udelay(1);
	stm32_gpio_set(IO_LCD_EN);
	udelay(1);
	stm32_gpio_clr(IO_LCD_EN);
};

static void lcd_wr(unsigned int data)
{
	/* data out */
	stm32_gpio_set(IO_LCD_RS);
	lcd_out(data);
}
...
struct lcd_dev * lcd20x4_init(void)
{
	...
	stm32_gpio_mode(IO_LCD_POWER, OUTPUT, PUSH_PULL| SPEED_MED);
	stm32_gpio_clr(IO_LCD_POWER);
	stm32_gpio_mode(IO_LCD_BACKLIGHT, OUTPUT, PUSH_PULL| SPEED_MED);
	stm32_gpio_clr(IO_LCD_BACKLIGHT);
	...
	stm32_gpio_mode(IO_LCD_RS, OUTPUT, PUSH_PULL| SPEED_MED);

	stm32_gpio_mode(IO_LCD_D0, OUTPUT, PUSH_PULL| SPEED_MED);
	stm32_gpio_mode(IO_LCD_D1, OUTPUT, PUSH_PULL| SPEED_MED);
	stm32_gpio_mode(IO_LCD_D2, OUTPUT, PUSH_PULL| SPEED_MED);
	stm32_gpio_mode(IO_LCD_D3, OUTPUT, PUSH_PULL| SPEED_MED);
	stm32_gpio_mode(IO_LCD_D4, OUTPUT, PUSH_PULL| SPEED_MED);
	stm32_gpio_mode(IO_LCD_D5, OUTPUT, PUSH_PULL| SPEED_MED);
	stm32_gpio_mode(IO_LCD_D6, OUTPUT, PUSH_PULL| SPEED_MED);
	stm32_gpio_mode(IO_LCD_D7, OUTPUT, PUSH_PULL| SPEED_MED);

	stm32_gpio_clr(IO_LCD_EN);
	stm32_gpio_clr(IO_LCD_RNW);
	stm32_gpio_clr(IO_LCD_RS);
	stm32_gpio_set(IO_LCD_NRE);
	stm32_gpio_clr(IO_LCD_D0);
	stm32_gpio_clr(IO_LCD_D1);
	stm32_gpio_clr(IO_LCD_D2);
	stm32_gpio_clr(IO_LCD_D3);
	stm32_gpio_clr(IO_LCD_D4);
	stm32_gpio_clr(IO_LCD_D5);
	stm32_gpio_clr(IO_LCD_D6);
	stm32_gpio_clr(IO_LCD_D7);

	lcd_usleep(10);
	stm32_gpio_set(IO_LCD_POWER);
	stm32_gpio_set(IO_LCD_BACKLIGHT);
