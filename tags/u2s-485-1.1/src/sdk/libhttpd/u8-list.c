...
void u8_list_insert(struct list * lst, unsigned int key)
{
	int idx;
	int i;
...
bool u8_list_contains(struct list * lst, unsigned int key)
{
	return __key_lookup(lst, key) < 0 ? false : true;
}

int u8_list_indexof(struct list * lst, unsigned int key)
{
	return __key_lookup(lst, key);
}

void u8_list_remove(struct list * lst, unsigned int key)
{
	int idx;
...
void u8_list_subtract(struct list * from, struct list * lst)
{
	int i;
...
void u8_list_dump(FILE * f,  struct list * lst)
{
	int i;
...
void u8_list_init(struct list * lst)
{
	lst->cnt = 0;
}
