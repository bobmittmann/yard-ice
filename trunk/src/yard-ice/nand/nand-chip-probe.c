...
static uint16_t onfi_crc(uint8_t * data)
{
...
static int nand_onfi_get_info(const void * arg, nand_chip_t * chip,
...
static int nand_lookup_info(const void * arg, nand_chip_t * chip,
							const nand_io_op_t * io, uint8_t * id)
{
...
nand_chip_t *  nand_chip_probe(nand_dev_t * nand, unsigned int chip_id)
{
	...
	chip = &nand->chip[chip_id];
	io = nand->io;

	chip->tmo_ms = 100;
	chip->iobase = nand->iobase;
	...
	if (to_uint32_le(buf) == NANDONFI_ID) {
		...
		if ((ret = nand_onfi_get_info(arg, chip, io)) < 0) {
			DCC_LOG(LOG_WARNING, "nand_onfi_get_info() fail!");
		}
	} else {
	...
	if (ret < 0) {
		if ((ret = nand_lookup_info(arg, chip, io, id)) < 0) {
			DCC_LOG(LOG_WARNING, "nand_lookup_info() fail!");
			return NULL;
		}
	}

	if ((ret = nand_chip_init(nand, chip)) < 0) {
		DCC_LOG(LOG_WARNING, "nand_reset() fail!");
		return NULL;
	}
