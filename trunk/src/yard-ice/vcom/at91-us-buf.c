...
void __attribute__((noreturn)) at91_us_isr(struct at91_us_dev * dev)
{
	...
	for (;;) {
		...
		if (csr & US_ENDRX) {
			DCC_LOG(LOG_TRACE, "DMA complete");
			n = 1 - n;
			us->rpr = dev->rx_buf[n];
			us->rcr = 64;
			us->cr = US_STTTO;
		...
		if (csr & US_TIMEOUT) {
			...
			us->cr = US_STTTO;
			if ((n = len - us->rcr) == 0) {
				DCC_LOG(LOG_TRACE, "0 bytes !!!");
				continue;
			}
			/* release the DMA channel */
			us->rcr = 0;
			len = n;
			DCC_LOG(LOG_TRACE, "DMA timeout!");
			break;
		}
	}
...
int at91_us_write(struct at91_us_dev * dev, const void * buf, int len)
{
	...
	DCC_LOG3(LOG_TRACE, "buf=%p len=%d sp=%p", buf, len, __sp());
	...
	if (len == 1) {
		...
		while (!(us->csr & US_ENDTX)) {
			us->ier = US_ENDTX;
			DCC_LOG(LOG_TRACE, "wait ENDTX 1");
			uthread_cond_wait(dev->tx_cond, dev->mutex);
		}
		...
		do {
			DCC_LOG(LOG_TRACE, "wait ENDTX 2");
			us->ier = US_ENDTX;
			uthread_cond_wait(dev->tx_cond, dev->mutex);
		} while (!(us->csr & US_ENDTX));
...
int at91_us_read(struct at91_us_dev * dev, void * buf, int len, int msec)
{
	...
	DCC_LOG2(LOG_TRACE, "buf=0x%p len=%d", buf, len);

	DCC_LOG1(LOG_TRACE, "msec=%d", msec);
	...
	while (!(us->csr & US_ENDRX)) {
		DCC_LOG(LOG_TRACE, "receiving???");
		us->ier = US_ENDRX | US_TIMEOUT;
		if (uthread_cond_timedwait(dev->rx_cond, dev->mutex, msec) < 0) {
			if (errno == ETIMEDOUT) {
				uthread_mutex_unlock(dev->mutex);
				DCC_LOG(LOG_TRACE, "TIMEOUT 2");
				return 0;
			}
		}
	}
	...
	/* configure DMA */
	DCC_LOG(LOG_TRACE, "configure DMA");
	us->rpr = buf;
	us->rcr = len;
	...
	for (;;) {
		DCC_LOG(LOG_TRACE, "enable IRQ");
		us->ier = US_ENDRX | US_TIMEOUT;
		
		if (uthread_cond_timedwait(dev->rx_cond, dev->mutex, msec) < 0) {
			if (errno == ETIMEDOUT) {
				DCC_LOG(LOG_TRACE, "TIMEOUT 3");
				us->rcr = 0;
				uthread_mutex_unlock(dev->mutex);
				return 0;
			}
...
//	mr |= US_CHMODE_LOCAL_LOOPBACK;
	...
	DCC_LOG2(LOG_TRACE, "brg=%d rtor=%d", us->brgr, us->rtor);
