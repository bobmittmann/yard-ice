...
int ice_comm_read(ice_comm_t * comm, void * buf, int len, int tmo)
{
	...
	memcpy(buf, &comm->rx_buf[head], n * 4);
	memcpy(((uint8_t *)buf) + n * 4, comm->rx_buf, m * 4);
...
//	for (i = 0; i < (len / 4); i++) {
	for (n = 0; n < len; n++) {
		if ((head - comm->tx_tail) == ICE_COMM_TX_BUF_LEN) {
			DCC_LOG(LOG_MSG, "block");
			/* wait the end of operation */
...
//			(src[i + 2] << 8) | (src[i + 3] << 12);
		...
		comm->tx_buf[head & (ICE_COMM_TX_BUF_LEN - 1)] = val;
		comm->tx_head = ++head;
	}
