...
static inline int __cm3rtos_sched(void)
{
 ...
 bmp_bit_wr(&cm3rtos_rt.ready, idx, act);
...

{
 ...
 while ((j = cm3_clz(wq)) < 32) {
  ...
  if ((int32_t)(cm3rtos_rt.clock[j] - ticks) <= 0) {
   ...
   bmp_bit_set(&cm3rtos_rt.ready, j);
 ...
 if (idx != 32) {
  ...
  if (cm3rtos_rt.sched_val[idx] < 0) {
   ...
   bmp_bit_set(&cm3rtos_rt.cpu_wait, idx);

  } else {
   bmp_bit_set(&cm3rtos_rt.ready, idx);
...
int cm3rtos_cancel_svc(unsigned int idx, int code)
{
 ...
 if (bmp_bit_rd(&cm3rtos_rt.th_alloc, idx) == 0) {
  printf("%s(): <%d> cancel %d failed, not alloc!\n",
      __func__, cm3rtos_rt.active >> 1, idx);
  ret = -1;
 } else {
  bmp_bit_clr((void *)&cm3rtos_rt.cpu_wait, idx);
  bmp_bit_clr((void *)&cm3rtos_rt.wq_clock, idx);
...
void cm3rtos_sleep_svc(unsigned int ms)
{
 ...
 cm3rtos_rt.clock[idx] = cm3rtos_rt.ticks + (ms / 1);

 bmp_bit_set(&cm3rtos_rt.wq_clock, idx);
...
int cm3rtos_init(void)
{
 struct cm3_systick * systick = ((struct cm3_systick *) ((0xe000e000) + 0x0010));
 int idx;
 int irq;
 ...
 cm3_except_pri_set(11, 0);


 cm3_except_pri_set(15, 128);

 cm3_except_pri_set(14, 128);

 for (irq = 0; irq < 80; irq++)
 ...
 cm3_control_set((1 << 1) | (0 << 0));


 cm3rtos_rt.th_alloc = 0xffffffff << 8;
 ...
 cm3rtos_rt.active = (idx << 1) + 1;
 cm3rtos_rt.sched_limit = cm3rtos_rt.sched_pri[idx] = 1;


 systick->load = (15000000 / 1000) - 1;
 systick->val = 0;
 systick->ctrl = (0x01 << 0) | (0x01 << 1);
