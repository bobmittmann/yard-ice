...
void __attribute__((aligned(16))) cm3_systick_isr(void)
{
	...
	while ((j = cm3_clz(wq)) < 32) {
		...
		if ((int32_t)(cm3rtos_rt.clock[j] - ticks) <= 0) {
			...
			stat = cm3rtos_rt.th_stat[j];
			cm3rtos_rt.th_stat[j] = 0;
			/* remove from other wait queue, if any */
			bmp_bit_clr(&cm3rtos_rt.wq_lst[stat >> 1], j);  
#endif
...
//			printf("^%d", j);
	...
	if (idx != CM3RTOS_IDLE_IDX) {
		...
		if (cm3rtos_rt.sched_val[idx] < 0) {
			...
			bmp_bit_set(&cm3rtos_rt.wq_tmshare, idx);  
//			printf(" w%x", cm3rtos_rt.wq_tmshare);
		} else {
...
void __attribute__((noreturn)) cm3rtos_thread_exit(int code)
{
	...
	int i;
	/* remove from other wait queue, if any */
	for (i = 0; i < __cm3rtos_wq_idx(cm3rtos_rt.wq_end); i++) {
		bmp_bit_clr(&cm3rtos_rt.wq_lst[i], idx);  
	}
#endif
#endif
