...
/* Bits 7:0 DR[7:0] 8-bit data register
   ...
   maintained if the next data to be transmitted is put in DR once 
   the transmission is started (TxE=1)
   –Receiver mode: Received byte is copied into DR (RxNE=1). A continuous 
   transmit stream can be maintained if DR is read before the next data byte 
   is received (RxNE=1).
   Note: In slave mode, the address is not copied into DR.
   Note: Write collision is not managed (DR can be written if TxE=0).
   Note: If an ARLO event occurs on ACK pulse, the received byte is not 
   copied into DR and so cannot be read. */
...
/* Note: fPCLK1 must be at least 2 MHz to achieve standard mode I²C 
   ...
   frequencies. It must be a multiple of 10MHz to reach the 400 kHz 
   maximum I²C fast mode clock. 
   The CCR register must be configured only when the I2C 
   is disabled (PE = 0). */
...
For instance: in standard mode, to generate a 100 kHz SCL frequency:
	...
	Note: 1. The minimum allowed value is 0x04, except in FAST DUTY 
	mode where the minimum allowed value is 0x01. 
	These timings are without filters.
	The CCR register must be configured only when 
...
ts 15:5 Reserved, must be kept at reset value */
